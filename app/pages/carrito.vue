...
<template>
  <main class="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
    ...
    <section v-if="hasHydrated && items.length === 0"
      class="mt-10 rounded-3xl border border-dashed border-espresso/20 bg-white/45 px-6 py-16 text-center sm:px-12">
      ...
      <NuxtLink to="/catalogo"
        class="mt-7 inline-flex rounded-full bg-espresso px-6 py-3 text-sm font-semibold text-mascarpone transition-colors hover:bg-cocoa">
        Ver sabores
      </NuxtLink>
    </section>
    ...
    <div v-else-if="hasHydrated" class="mt-10 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_24rem]">
      <div class="space-y-8">
        <section aria-labelledby="items-title">
          <div class="flex items-baseline justify-between gap-4">
            ...
            <span class="font-mono text-xs uppercase tracking-wider text-espresso/45">{{ items.length }}
              referencias</span>
          </div>

          <ul class="mt-4 divide-y divide-espresso/10 border-y border-espresso/10 bg-white/35">
            <li v-for="item in items" :key="item.id"
              class="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-4 py-5 sm:grid-cols-[5.5rem_minmax(0,1fr)_auto] sm:gap-5">
              <NuxtLink :to="`/catalogo/${item.slug}`"
                class="aspect-square overflow-hidden rounded-2xl bg-linear-to-br from-mascarpone to-espresso/10">
                <img v-if="item.image_url" :src="item.image_url" :alt="item.name" class="h-full w-full object-cover" />
                <span v-else class="flex h-full items-center justify-center font-display text-3xl text-espresso/25">{{
                  item.name.charAt(0) }}</span>
              </NuxtLink>

              <div class="min-w-0">
                <NuxtLink :to="`/catalogo/${item.slug}`"
                  class="font-display text-xl font-semibold text-espresso hover:text-cocoa">
                  {{ item.name }}
                </NuxtLink>
                <p class="mt-1 font-mono text-xs text-espresso/55">{{ formatCOP(item.price_cents) }} por unidad</p>
                <div class="mt-4 flex items-center gap-3">
                  <div class="inline-flex items-center rounded-full border border-espresso/15 bg-white"
                    aria-label="Cantidad">
                    <button type="button"
                  ...
                  <button type="button" class="text-xs font-semibold text-cocoa underline-offset-4 hover:underline"
                    @click="removeItem(item.id)">Quitar</button>
                </div>
              </div>

              <p class="col-start-2 font-mono text-sm font-medium text-espresso sm:col-start-auto sm:pt-1">{{
                formatCOP(item.price_cents * item.quantity) }}</p>
            </li>
          </ul>
        </section>
        ...
        <form class="space-y-8" @submit.prevent="preparePayment">
          <section aria-labelledby="delivery-title">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-2">
              <label class="relative flex cursor-pointer gap-3 rounded-2xl border bg-white/60 p-4 transition-colors"
                :class="fulfillment === 'pickup' ? 'border-cocoa bg-white ring-1 ring-cocoa/25' : 'border-espresso/15 hover:border-espresso/30'">
                <input v-model="fulfillment" value="pickup" type="radio" name="fulfillment" class="sr-only" />
                <span
                  class="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-botanical text-sm text-mascarpone">↗</span>
                <span>
                  ...
                  <span v-if="config.public.pickupAddress" class="mt-2 block text-xs text-espresso/55">{{
                    config.public.pickupAddress }}</span>
                  <span v-if="config.public.pickupHours" class="mt-1 block text-xs text-espresso/55">{{
                    config.public.pickupHours }}</span>
                </span>
              </label>

              <label class="relative flex cursor-pointer gap-3 rounded-2xl border bg-white/60 p-4 transition-colors"
                :class="fulfillment === 'delivery' ? 'border-cocoa bg-white ring-1 ring-cocoa/25' : 'border-espresso/15 hover:border-espresso/30'">
                <input v-model="fulfillment" value="delivery" type="radio" name="fulfillment" class="sr-only" />
                <span
                  class="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-sky text-sm text-espresso">⌂</span>
                <span>
          ...
          <section aria-labelledby="contact-title">
            ...
            <div class="mt-4 grid gap-4 sm:grid-cols-2">
              <label class="block text-sm font-semibold text-espresso">Nombre
                <input v-model="contactName" required autocomplete="name" placeholder="Tu nombre"
              ...
              <label class="block text-sm font-semibold text-espresso">Celular
                <input v-model="phone" required autocomplete="tel" inputmode="tel" placeholder="300 000 0000"
                  class="mt-2 w-full rounded-xl border border-espresso/15 bg-white/70 px-3 py-3 text-sm font-normal outline-none transition placeholder:text-espresso/35 focus:border-cocoa focus:ring-2 focus:ring-cocoa/20" />
              </label>
            </div>
      ...
      <aside
        class="sticky top-24 overflow-hidden rounded-3xl border border-espresso/10 bg-espresso text-mascarpone shadow-xl shadow-espresso/10">
        <div class="p-6">
          ...
          <div class="mt-5 border-t border-mascarpone/20 pt-5">
            <div class="flex items-end justify-between gap-4"><span class="font-semibold">Total</span><span
                class="font-mono text-2xl font-semibold text-saffron">{{ formatCOP(totalCents) }}</span></div>
          </div>
        </div>
